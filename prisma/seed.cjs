...
const schemes = [
  {
    code: 'PCM-HSE-F',
    title: 'Professionnel Certifié HSE Minier — Fondation',
    description: 'Fondamentaux HSE applicables aux opérations minières.',
  ...
  {
    code: 'PCM-II',
    title: 'Professionnel Certifié en Investigation d’Incidents Miniers',
    description: 'Méthodes d’investigation, causes racines et actions correctives.',
  ...
  {
    code: 'PCM-ER',
    title: 'Professionnel Certifié en Évaluation des Risques Miniers',
    description: 'Identification des dangers, évaluation et maîtrise des risques miniers.',
...
async function main() {
  for (const item of schemes) {
    const scheme = await prisma.certificationScheme.upsert({
      ...
      update: { title: item.title, description: item.description, active: true },
      create: { code: item.code, title: item.title, description: item.description, validityMonths: 36, passingScore: 75, maxAttempts: 2 },
    });
    const course = await prisma.course.upsert({
      where: { slug: item.course.slug },
      update: { ...item.course, published: true },
      create: { ...item.course, published: true },
    });
    let assessment = await prisma.assessment.findFirst({ where: { courseId: course.id, schemeId: scheme.id } });
    if (!assessment) {
      assessment = await prisma.assessment.create({ data: { courseId: course.id, schemeId: scheme.id, title: `Examen — ${item.title}`, type: 'EXAM', passingScore: 75, maxAttempts: 2, durationMinutes: 45, published: true } });
    }
    const count = await prisma.question.count({ where: { assessmentId: assessment.id } });
    if (count === 0) {
      await prisma.question.createMany({ data: item.questions.map((q, i) => ({ assessmentId: assessment.id, prompt: q[0], type: q[1].length === 2 && q[1].includes('Vrai') ? 'TRUE_FALSE' : 'SINGLE_CHOICE', options: q[1], correctAnswers: q[2], points: 1, sortOrder: i + 1 })) });
    }
  }
}
