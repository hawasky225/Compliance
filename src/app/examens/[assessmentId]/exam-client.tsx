...
export default function ExamClient({ assessmentId }: { assessmentId: string }) {
  ...
  if (result) return <section className="card exam-result"><p className="eyebrow">RÉSULTAT</p><h2>{result.passed ? 'Examen réussi' : 'Examen non réussi'}</h2><div className="score-big">{result.score}%</div><p>{result.passed ? 'Votre résultat satisfait au seuil requis.' : 'Vous pourrez utiliser une nouvelle tentative si elle est disponible.'}</p>{result.certification && <div className="success-panel"><strong>Certification délivrée</strong><span>{result.certification.certificateNumber}</span></div>}</section>;
